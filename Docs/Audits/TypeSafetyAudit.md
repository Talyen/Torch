...
- Keep external/browser decoding at the boundary (`src/game/input-bindings.ts`,
  `src/sim/*-save.ts`, and platform save adapters); do not scatter casts through
  `src/sim/**`.
...
- `src/sim/world-save.ts` and `src/sim/profile-save.ts` are the current
  versioned save decoders. Keep their validation as the single boundary rather
  than adding call-site casts; future migrations should extend that boundary.
...
- **Unsafe persistence/input boundaries:** casts around JSON parsing,
  `localStorage`, content/asset lookup, or save-provider adapters; confirm
  whether a runtime check or a typed owner is missing.
