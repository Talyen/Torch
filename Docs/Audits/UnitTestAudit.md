...
| Save/replay codecs, deterministic restoration, and provider failure handling                     | `src/sim/*-save.ts`, `src/sim/replay.ts`, `src/game/session.ts` → `tests/save-replay.test.ts`, `tests/session-save.test.ts` |
| Inventory, crafting, stations, equipment, and tool loadouts                                      | `src/sim/crafting.ts`, `src/sim/loadout.ts` → `tests/crafting.test.ts`, `tests/loadout.test.ts`                             |
| Journal state and profile/world projections                                                      | `src/sim/journal.ts`, `src/sim/profile-save.ts` → `tests/journal.test.ts`                                                   |
| Rendered menu/HUD, focus, responsive geometry, assets, theme                                     | `tests/e2e/first-light.spec.ts`, `tests/e2e/theme.spec.ts`, `tests/e2e/crafting.spec.ts`, and `tests/e2e/loadout.spec.ts`   |
